...
HEADERS={
    'User-Agent':'BoatRaceAI-FreeDashboard/2.12',
        ...
        for rk,race in ((stadium or {}).get('races') or {}).items():
            ...
            for ek,racer in (race.get('racers') or {}).items():
                ...
                rows.append({
                    ...
                    'lane':int(ek),
                    'racer_id':racer.get('number'),
                    'racer_name':racer.get('name'),
                    'rank_number':racer.get('rank_number'),
                    'age':racer.get('age'),
                    'weight':racer.get('weight'),
                    'flying_count':racer.get('flying_count'),
                    'late_count':racer.get('late_count'),
                    'avg_start':racer.get('average_start_timing'),
                    'racer_win_rate':racer.get('national_win_rate'),
                    'racer_2ren_rate':_pct(racer.get('national_top_2_percent')),
                    'racer_3ren_rate':_pct(racer.get('national_top_3_percent')),
                    'local_win_rate':racer.get('local_win_rate'),
                    'local_2ren_rate':_pct(racer.get('local_top_2_percent')),
                    'motor_no':racer.get('motor_number'),
                    'motor_2ren_rate':_pct(racer.get('motor_top_2_percent')),
                    'boat_no':racer.get('boat_number'),
                    'boat_2ren_rate':_pct(racer.get('boat_top_2_percent')),
